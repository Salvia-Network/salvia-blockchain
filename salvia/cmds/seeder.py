...
import salvia.cmds.configure as salvia_configure
from salvia import __version__
from salvia.cmds.salvia import monkey_patch_click
from salvia.cmds.init_funcs import init
from salvia.seeder.util.config import patch_default_seeder_config
from salvia.seeder.util.service_groups import all_groups, services_for_groups
from salvia.seeder.util.service import launch_service, kill_service
...
@click.group(
    help=f"\n  Manage the Salvia Seeder ({__version__})\n",
    epilog="Try 'salvia seeder start crawler' or 'salvia seeder start server'",
...
@cli.command("version", short_help="Show the Salvia Seeder version")
...
def init_cmd(ctx: click.Context, **kwargs):
    print("Calling Salvia Seeder Init...")
    init(None, ctx.obj["root_path"], True)
    if os.environ.get("SALVIA_ROOT", None) is not None:
        print(f"warning, your SALVIA_ROOT is set to {os.environ['SALVIA_ROOT']}.")
    root_path = ctx.obj["root_path"]
    print(f"Salvia directory {root_path}")
    if root_path.is_dir() and not Path(root_path / "config" / "config.yaml").exists():
        # This is reached if SALVIA_ROOT is set, but there is no config
        # This really shouldn't happen, but if we dont have the base salvia config, we can't continue
...
def configure(
    ...
    salvia_configure.configure(root_path, "", "", "", "", "", "", "", "", testnet, "")
    ...
    if testnet is not None:
        if testnet == "true" or testnet == "t":
            ...
            print("Updating Salvia Seeder to mainnet settings")
            port = 8444
            network = "mainnet"
            bootstrap = ["node.salvianetwork.net"]
    ...
    if change_made:
        print("Restart any running Salvia Seeder services for changes to take effect")
