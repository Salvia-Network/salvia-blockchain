...
from salvia.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if "xslv_target_address" not in config["farmer"]:
        print(
            f"Setting the xslv destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
    ...
    if "xslv_target_address" not in config["pool"]:
        print(f"Setting the xslv destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
    ...
    if updated_target:
        print(
            f"To change the XSLV destination addresses, edit the `xslv_target_address` entries in"
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                salvia_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return salvia_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)
