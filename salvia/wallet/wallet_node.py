...
from salvia.consensus.blockchain import ReceiveBlockResult
...
from salvia.full_node.weight_proof import chunks
...
from salvia.server.peer_store_resolver import PeerStoreResolver
...
from salvia.types.weight_proof import WeightProof, SubEpochData
from salvia.util.byte_types import hexstr_to_bytes
from salvia.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from salvia.util.ints import uint32, uint64
from salvia.util.keychain import KeyringIsLocked, Keychain
from salvia.util.network import get_host_addr
from salvia.util.path import mkdir, path_from_root
from salvia.wallet.derivation_record import DerivationRecord
from salvia.wallet.util.wallet_sync_utils import (
    validate_additions,
    validate_removals,
...
class WalletNode:
    ...
    async def new_puzzle_hash_created(self, puzzle_hashes: List[bytes32]):
        if len(puzzle_hashes) == 0:
            return
        assert self.server is not None
        full_nodes: Dict[bytes32, WSSalviaConnection] = self.server.connection_by_type.get(NodeType.FULL_NODE, {})
    ...
    def on_disconnect(self, peer: WSSalviaConnection):
    ...
    async def trusted_sync(self, full_node: WSSalviaConnection):
        """
    ...
    async def subscribe_to_phs(self, puzzle_hashes: List[bytes32], peer: WSSalviaConnection, height=uint32(0)):
        """
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSSalviaConnection):
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_last_tx_from_peer(self, height: uint32, peer: WSSalviaConnection) -> Optional[HeaderBlock]:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSSalviaConnection, peak: HeaderBlock
    ...
    async def untrusted_sync_to_peer(
        self, peer: WSSalviaConnection, weight_proof: WeightProof, syncing: bool, fork_height: int
    ):
        assert self.wallet_state_manager is not None
