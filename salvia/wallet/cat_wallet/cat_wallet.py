...
from salvia.protocols.wallet_protocol import PuzzleSolutionResponse, CoinState
...
from salvia.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from salvia.wallet.cat_wallet.cat_info import CATInfo
from salvia.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    SpendableCAT,
...
from salvia.wallet.payment import Payment
from salvia.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
...
from salvia.wallet.util.wallet_types import WalletType, AmountWithPuzzlehash
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            salvia_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        salvia_tx = dataclasses.replace(salvia_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(salvia_tx)
    ...
    async def create_tandem_xch_tx(
        ...
        if fee > amount_to_claim:
            salvia_coins = await self.standard_wallet.select_coins(fee)
            origin_id = list(salvia_coins)[0].name()
            salvia_tx = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
            ...
            assert salvia_tx.spend_bundle is not None

            message = None
            for spend in salvia_tx.spend_bundle.coin_spends:
            ...
            salvia_coins = await self.standard_wallet.select_coins(fee)
            selected_amount = sum([c.amount for c in salvia_coins])
            salvia_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XSLV as part of this
        regular_salvia_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_salvia_to_claim = payment_amount

        need_salvia_transaction = (fee > 0 or regular_salvia_to_claim > 0) and (fee - regular_salvia_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                first = False
                if need_salvia_transaction:
                    if fee > regular_salvia_to_claim:
                        announcement = Announcement(coin.name(), b"$", b"\xca")
                        salvia_tx, _ = await self.create_tandem_xch_tx(
                            fee, uint64(regular_salvia_to_claim), announcement_to_assert=announcement
                        )
                    ...
                    elif regular_salvia_to_claim > fee:
                        salvia_tx, _ = await self.create_tandem_xch_tx(fee, uint64(regular_salvia_to_claim))
                        innersol = self.standard_wallet.make_solution(
        ...
        salvia_spend_bundle = SpendBundle([], G2Element())
        if salvia_tx is not None and salvia_tx.spend_bundle is not None:
            salvia_spend_bundle = salvia_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, salvia_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
        ...
        if salvia_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=salvia_tx.confirmed_at_height,
                    created_at_time=salvia_tx.created_at_time,
                    to_puzzle_hash=salvia_tx.to_puzzle_hash,
                    amount=salvia_tx.amount,
                    fee_amount=salvia_tx.fee_amount,
                    confirmed=salvia_tx.confirmed,
                    sent=salvia_tx.sent,
                    spend_bundle=None,
                    additions=salvia_tx.additions,
                    removals=salvia_tx.removals,
                    wallet_id=salvia_tx.wallet_id,
                    sent_to=salvia_tx.sent_to,
                    trade_id=salvia_tx.trade_id,
                    type=salvia_tx.type,
                    name=salvia_tx.name,
                    memos=[],
                )
