...
from salvia.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from salvia.plotters.chiapos import get_chiapos_install_info, plot_salvia
from salvia.plotters.madmax import get_madmax_install_info, plot_madmax
from salvia.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in SALVIA_ROOT.
    salvia_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within SALVIA_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, salvia_plotter, "chiapos", "Chiapos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_salvia(args, salvia_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, salvia_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, salvia_root_path, root_path)
