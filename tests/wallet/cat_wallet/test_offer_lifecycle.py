...
from salvia.clvm.spend_sim import SpendSim, SimClient
...
from salvia.wallet.trading.offer import Offer, NotarizedPayment
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            salvia_coins: List[Coin] = all_coins[None]
            red_coins: List[Coin] = all_coins["red"]
            blue_coins: List[Coin] = all_coins["blue"]

            # Create an XSLV Offer for RED
            salvia_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            salvia_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                salvia_requested_payments, salvia_coins
            )
            salvia_announcements: List[Announcement] = Offer.calculate_announcements(salvia_requested_payments)
            salvia_secured_bundle: SpendBundle = self.generate_secure_bundle(salvia_coins, salvia_announcements, 1000)
            salvia_offer = Offer(salvia_requested_payments, salvia_secured_bundle)
            assert not salvia_offer.is_valid()

            # Create a RED Offer for XSLV
            ...
            new_offer = Offer.aggregate([salvia_offer, red_offer])
            ...
            # Create yet another offer of BLUE for XSLV and RED
            ...
            assert new_offer.summary() == (
                ...
                {"xslv": 900, str_to_tail_hash("red").hex(): 350},
            )
