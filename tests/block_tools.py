...
from salvia.cmds.init_funcs import create_all_ssl, create_default_salvia_config
from salvia.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
from salvia.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from salvia.util.errors import Err
from salvia.full_node.generator import setup_generator_args
from salvia.full_node.mempool_check_conditions import GENERATOR_MOD
from salvia.plotting.create_plots import create_plots, PlotKeys
...
from salvia.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from salvia.plotting.util import PlotInfo, PlotsRefreshParameter, PlotRefreshResult, parse_plot_info
...
from salvia.types.blockchain_format.program import INFINITE_COST
...
from salvia.util.ints import uint8, uint16, uint32, uint64, uint128
from salvia.util.keychain import Keychain, bytes_to_mnemonic
...
from salvia.util.vdf_prover import get_vdf_info_and_proof
...
class BlockTools:
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="salvia-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `salvia generate keys`")
    ...
    async def setup_plots(self):
        ...
        try:
            ...
            plot_keys_2 = PlotKeys(self.farmer_pk, None, encode_puzzle_hash(self.pool_ph, "xslv"))
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("SALVIA_ROOT", "~/.salvia/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
