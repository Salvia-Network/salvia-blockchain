...
from salvia.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from salvia.full_node.full_node_api import FullNodeAPI
from salvia.server.start_farmer import service_kwargs_for_farmer
from salvia.server.start_full_node import service_kwargs_for_full_node
from salvia.server.start_harvester import service_kwargs_for_harvester
from salvia.server.start_introducer import service_kwargs_for_introducer
from salvia.server.start_service import Service
from salvia.server.start_timelord import service_kwargs_for_timelord
from salvia.server.start_wallet import service_kwargs_for_wallet
from salvia.simulator.start_simulator import service_kwargs_for_full_node_simulator
from salvia.timelord.timelord_launcher import kill_processes, spawn_process
...
from salvia.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["xslv_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xslv")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xslv_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xslv")
