...
from salvia.util.config import create_default_salvia_config, initial_config_file, load_config, save_config
...
class TestConfig:
    ...
    def test_create_config_new(self, tmpdir):
        """
        Test create_default_salvia_config() as in a first run scenario
        """
    ...
    def test_create_config_overwrite(self, tmpdir):
        """
        Test create_default_salvia_config() when overwriting an existing config.yaml
        """
        # When: using a clean directory
        root_path: Path = Path(tmpdir)
        config_file_path: Path = root_path / "config" / "config.yaml"
    ...
    def test_load_config(self, root_path_populated_with_config, default_config_dict):
        ...
        assert (
            ...
            == "cd712de5a54aa420613d90b4c0f7ad28de3ca1f3edef0ef3fe12d2721c067802"
        )
