...
if [ "$(uname -m)" = "armv7l" ]; then
  echo ""
	echo "WARNING:"
	echo "The Salvia Blockchain requires a 64 bit OS and this is 32 bit armv7l"
	echo "For more information, see"
	echo "https://github.com/Salvia-Network/salvia-blockchain/wiki/Raspberry-Pi"
	echo "Exiting."
	exit 1
fi
# Get submodules
			...
			if [ "$BEST_VERSION" = "" ]; then
				...
				if [ "$BEST_VERSION" = "3" ]; then
					...
					if [[ "$PY3_VERSION" =~ 3.10.* ]]; then
						echo "Salvia requires Python version <= 3.9.9"
...
echo "https://github.com/Salvia-Network/salvia-blockchain/wiki/Quick-Start-Guide"
echo ""
echo "To install the GUI type 'sh install-gui.sh' after '. ./activate'."
echo ""
echo "Type '. ./activate' and then 'salvia init' to begin."
