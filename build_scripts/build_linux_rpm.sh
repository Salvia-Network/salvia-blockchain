...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="salvia-blockchain-linux-x64"
else
	#PLATFORM="$1"
	DIR_NAME="salvia-blockchain-linux-arm64"
fi
...
electron-packager . salvia-blockchain --asar.unpack="**/daemon/**" --platform=linux \
--icon=src/assets/img/Salvia.icns --overwrite --app-bundle-id=net.salvia.blockchain \
--appVersion=$SALVIA_INSTALLER_VERSION --executable-name=salvia-blockchain
LAST_EXIT_CODE=$?
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create salvia-blockchain-$SALVIA_INSTALLER_VERSION.rpm"

	# shellcheck disable=SC2046
	NODE_ROOT="$(dirname $(dirname $(which node)))"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Salvia-Network/salvia-blockchain/issues/3846
  ...
  --arch "$REDHAT_PLATFORM" --options.version $SALVIA_INSTALLER_VERSION \
  --license ../LICENSE --options.bin salvia-blockchain --options.name salvia-blockchain
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
